...
app.get('/api/hikvision/status', forwardWithCookie('GET'));
app.post('/api/hikvision/poll', forwardWithCookie('POST'));
app.post('/api/hikvision/start', forwardWithCookie('POST'));
app.post('/api/hikvision/stop', forwardWithCookie('POST'));
...
// Lazada Phase 9.3/9.4/9.5 — OAuth + sync
app.get('/api/marketplace/lazada/status', forwardWithCookie('GET'));
app.get('/api/marketplace/lazada/auth-url', forwardWithCookie('GET'));
app.post('/api/marketplace/lazada/refresh-token', forwardWithCookie('POST'));
app.get('/api/marketplace/lazada/products', forwardWithCookie('GET'));
app.get('/api/marketplace/lazada/seller', forwardWithCookie('GET'));
app.post('/api/marketplace/lazada/sync-products', forwardWithCookie('POST'));
app.get('/api/marketplace/lazada/snapshot', forwardWithCookie('GET'));
app.get('/api/marketplace/lazada/match', forwardWithCookie('GET'));
app.post('/api/marketplace/lazada/apply-matches', forwardWithCookie('POST'));
app.get('/api/marketplace/lazada/mappings', forwardWithCookie('GET'));
...
app.post('/api/marketplace/lazada/import-orders', forwardWithCookie('POST'));
app.post('/api/marketplace/lazada/push-stock', forwardWithCookie('POST'));
app.post('/api/marketplace/lazada/import', forwardWithCookie('POST'));
...
async function buildStoreContext(userMessage, userId) {
  ...
  var ctx = '';
  // tier info for Gemini
  if (memberTier === 'vip') {
    ctx += 'customer_tier: VIP\n';
  } else if (memberTier === 'wholesale') {
    ctx += 'customer_tier: wholesale\n';
  } else if (memberTier) {
    ctx += 'customer_tier: ' + memberTier + '\n';
  }
  if (storeInfo) {
...
app.post('/api/bigseller/edit-queue', async function(req, res) {
  try {
    var r = await adminFetch(LOCAL_API_BASE + '/api/bigseller/edit-queue', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(req.body || {})
    });
    res.status(r.status).json(await r.json());
  } catch (err) { res.status(500).json({ error: err.message }); }
});
app.delete('/api/bigseller/edit-queue/:sku', async function(req, res) {
  try {
    var r = await adminFetch(LOCAL_API_BASE + '/api/bigseller/edit-queue/' + encodeURIComponent(req.params.sku), { method: 'DELETE' });
    res.status(r.status).json(await r.json());
  } catch (err) { res.status(500).json({ error: err.message }); }
});
app.delete('/api/bigseller/edit-queue', async function(req, res) {
  try {
    var r = await adminFetch(LOCAL_API_BASE + '/api/bigseller/edit-queue', { method: 'DELETE' });
    res.status(r.status).json(await r.json());
  } catch (err) { res.status(500).json({ error: err.message }); }
});
app.post('/api/bigseller/edit-queue/upload', async function(req, res) {
  try {
    var r = await adminFetch(LOCAL_API_BASE + '/api/bigseller/edit-queue/upload', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(req.body || {})
    });
    res.status(r.status).json(await r.json());
  } catch (err) { res.status(500).json({ error: err.message }); }
